...
def flow_list(request, **params):
    ...
    myflows = neutronclient(request).list_extensions(**params).get('myextensions')
    LOG.info(_('myflows%s')%myflows)
    # Get subnet list to expand subnet info in network list.
#     zsubnets = subnet_list(request)
...
def list_extensions(request):
    ...
    def list_myextensions(self, **_params):
        """Fetches a list of all myextensions for a tenant."""
        return self.get(self.myextensions_path,params=_params)

@memoized
def list_myextensions(request):
    flows = neutronclient(request).list_myextensions()
    return flows
